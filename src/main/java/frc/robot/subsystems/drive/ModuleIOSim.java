...
public class ModuleIOSim implements ModuleIO {
  ...
  public void updateInputs(ModuleIOInputs inputs) {
    ...
    inputs.driveCurrentAmps =
        new double[] {Math.abs(moduleSimulation.getDriveMotorSupplyCurrent().in(Amps))};
    ...
    inputs.turnPosition =
        new Rotation2d(
            moduleSimulation
                ...
                .divide(moduleSimulation.STEER_GEAR_RATIO));
    ...
    inputs.turnCurrentAmps =
        new double[] {Math.abs(moduleSimulation.getSteerMotorSupplyCurrent().in(Amps))};
    ...
    inputs.odometryTurnPositions =
        Arrays.stream(moduleSimulation.getCachedSteerRelativeEncoderPositions())
            .map(
                relativeEncoderPosition ->
                    new Rotation2d(
                        relativeEncoderPosition.divide(moduleSimulation.STEER_GEAR_RATIO)))
